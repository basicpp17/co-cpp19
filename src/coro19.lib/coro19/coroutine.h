...
template<typename _PromiseT = void> struct coroutine_handle;
...
template<typename _PromiseT> struct coroutine_handle : public coroutine_handle<void> {
    // 17.12.3.1, construct
...
template<> struct coroutine_handle<noop_coroutine_promise> : public coroutine_handle<void> {
...
#        if __has_builtin(__builtin_coro_noop)
    ...
    noop_coroutine_promise& promise() const noexcept {
        void* _Prom = __builtin_coro_promise(
            __builtin_coro_noop(), //
...
private:
    coroutine_handle() noexcept : coroutine_handle<void>{from_address(__builtin_coro_noop())} {}
#        else
...
#    elif defined(_MSC_VER)
    ...
    noop_coroutine_promise& promise() const noexcept { return *(noop_coroutine_promise*)(this->address()); }
...
template<class _Ret, class = void> struct coro_traits_sfinae {};

template<class _Ret> struct coro_traits_sfinae<_Ret, void_t<typename _Ret::promise_type>> {
...
template<typename P> struct coroutine_handle : public std::coroutine_handle<P> {};
...
template<typename _Ret, typename... _Ts> struct _Resumable_helper_traits {
    ...
    static void _Set_exception(void* _Addr) { _Promise_from_frame(_Addr)->set_exception(std::current_exception()); }
