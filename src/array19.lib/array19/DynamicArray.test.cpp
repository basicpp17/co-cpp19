...
#include "Array.h"
#include "DynamicArray.equals.h"
#include "DynamicArray.AsHex.ostream.h"
#include "DynamicArray.ostream.h"
...
TEST(DynamicArray, intExample) {

    auto v = DynamicArray<int>{};
    ...
    v.append(spanOfCArray({12, 23}));

    ASSERT_EQ(v.count(), 2u);
    EXPECT_GE(v.totalCapacity(), 2u);

    v.append(spanOfCArray({42, 64}));

    ASSERT_EQ(v.count(), 4u);

    v.splice(v.amend().begin() + 1, 2, spanOfCArray({99}));

    auto v2 = DynamicArray<int>{};
    v2.append(spanOfCArray({12, 99, 64}));
    ASSERT_EQ(v, v2);

    v2.append(spanOfCArray({45}));
    v2 = v; // copy
    ASSERT_EQ(v, v2);

    v.append(spanOfCArray({45}));
    v2 = std::move(v);
    ASSERT_EQ(spanOfCArray({12, 99, 64, 45}), Span{v2});
}

TEST(DynamicArray, construct) {
    auto v = DynamicArray{1, 2, 3};
...
struct NonTrivial;
using NonTrivialArray = DynamicArray<NonTrivial>;
...
TEST(DynamicArray, NontrivialExample) {
    auto v = DynamicArray<NonTrivial>{};
    ...
    v.append(spanOfCArray({NonTrivial{12}, NonTrivial{23}}));

    ASSERT_EQ(v.count(), 2u);
    EXPECT_GE(v.totalCapacity(), 2u);

    v.append(spanOfCArray({NonTrivial{42}, NonTrivial{64}}));

    ASSERT_EQ(v.count(), 4u);

    v.splice(v.amend().begin() + 1, 2, spanOfCArray({NonTrivial{99}}));

    auto v2 = DynamicArray<NonTrivial>{};
    v2.append(spanOfCArray({NonTrivial{12}, NonTrivial{99}, NonTrivial{64}}));
    ASSERT_EQ(v, v2);
}

TEST(DynamicArray, MoveAssignAfterMove) {
    using E = NonTrivial;
    using AA = DynamicArray<E>;
    auto v = AA{E{1}};
...
TEST(DynamicArray, CopyAssignAfterMove) {
    using E = NonTrivial;
    using AA = DynamicArray<E>;
    auto v = AA{E{1}};
...
TEST(DynamicArray, EmplaceStruct) {
    auto v = DynamicArray<Vec3>{};
    v.emplace_back(1.1, 2.2, 3.3);

    auto v2 = DynamicArray{Vec3{1.1, 2.2, 3.3}};
    EXPECT_EQ(v, v2);
}

TEST(DynamicArray, Ostream) {
    auto s = std::stringstream{};
    s << DynamicArray{10, 11, 16};
    EXPECT_EQ(s.str(), "[10, 11, 16]");
}

TEST(DynamicArray, OstreamStruct) {
    auto s = std::stringstream{};
    s << DynamicArray{Vec3{1.1, 2.2, 3.3}, Vec3{10, 11, 16}};
    EXPECT_EQ(s.str(), "[{x:1.1, y:2.2, z:3.3}, {x:10, y:11, z:16}]");
}

TEST(DynamicArray, OstreamAsHex) {
    auto s = std::stringstream{};
    s << AsHex{DynamicArray<uint8_t>{uint8_t{10}, uint8_t{11}, uint8_t{16}}};
...
TEST(DynamicArray, RecordedCopyAndMove) {
    using T = DynamicArray<Recorder>;
    auto v2 = T{};
    {
...
TEST(DynamicArray, RecordedSpliceGrowOverCapacity) {
    using T = DynamicArray<Recorder>;
...
13.ctor
    ...
    Recorder::store = std::stringstream{};
    ASSERT_EQ(v.unusedCapacity(), 0u) << "all capacity used";
    v.splice(v.amend().begin() + 1, 1, insert);
    EXPECT_EQ(Recorder::store.str(), R"(1.ctor(&&)
112.ctor(const&)
113.ctor(const&)
3.ctor(&&)
...
TEST(DynamicArray, RecordedSpliceShrink) {
    using T = DynamicArray<Recorder>;
...
13.ctor
    ...
    Recorder::store = std::stringstream{};
    v.splice(v.amend().begin() + 1, 3, insert);
...
TEST(DynamicArray, RecordedSpliceInsertOverEnd) {
    using T = DynamicArray<Recorder>;
...
TEST(DynamicArray, RecordedSpliceInsertBeforeEnd) {
    using T = DynamicArray<Recorder>;
...
TEST(DynamicArray, RecordedSpliceRemoveFirst) {
    using T = DynamicArray<Recorder>;
...
TEST(DynamicArray, RecordedSpliceRemoveLast) {
    using T = DynamicArray<Recorder>;
